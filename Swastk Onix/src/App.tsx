...
import logo from 'figma:asset/78a2b92e988ce22bb80d2c2d0edc8c797809cd81.png';

export default function App() {
  const [isBotOpen, setIsBotOpen] = useState(false);
  const [initialMessage, setInitialMessage] = useState<string>('');

  const handleOpenBotWithMessage = (message: string) => {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0B] text-white overflow-x-hidden">
      ...
      <div className="relative z-10">
        <Navbar />
        <Hero onOpenBot={() => setIsBotOpen(true)} onOpenBotWithMessage={handleOpenBotWithMessage} />
        <Services onOpenBotWithMessage={handleOpenBotWithMessage} />
        <Footer />
      </div>

      {/* Unix Bot */}
      <UnixBot isOpen={isBotOpen} setIsOpen={setIsBotOpen} initialMessage={initialMessage} />
    </div>
  );
}
        ...
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          >
            ...
            <div className="w-10 h-10">
              <img src={logo} alt="Swastik Onix Logo" className="w-full h-full object-contain" />
            </div>
...
function Hero({ onOpenBot, onOpenBotWithMessage }: { onOpenBot: () => void; onOpenBotWithMessage: (message: string) => void }) {
  const services = ['Web Development', 'App Development', 'Unity Game Dev', 'Marketing', 'Event Management'];
          ...
          <div className="flex flex-wrap justify-center gap-3 sm:gap-4 text-sm sm:text-base">
            {services.map((service, index) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => onOpenBotWithMessage(`I need ${service}`)}
                className="px-4 py-2 rounded-full bg-white/5 backdrop-blur-sm border border-white/10 text-gray-300 hover:border-[#00F2FF]/50 transition-colors cursor-pointer"
              >
                {service}
              </motion.button>
            ))}
...
function Services({ onOpenBotWithMessage }: { onOpenBotWithMessage?: (message: string) => void }) {
  const services = [
    {
      icon: Code2,
    ...
    <section className="py-20 sm:py-32 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {services.map((service, index) => (
            <ServiceCard key={service.title} service={service} index={index} onOpenBotWithMessage={onOpenBotWithMessage} />
          ))}
        </div>
      </div>
    </section>
...
function ServiceCard({ service, index, onOpenBotWithMessage }: { service: any; index: number; onOpenBotWithMessage?: (message: string) => void }) {
  const [isHovered, setIsHovered] = useState(false);

  const handleClick = () => {
    if (onOpenBotWithMessage) {
      onOpenBotWithMessage(`I need ${service.title}`);
    }
  };
